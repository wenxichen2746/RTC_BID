...
def main(
    ...
    def test_methods(config,levels,env,vel_target,inference_delay,execute_horizon,test_noise_std):
        ...
        if config.num_evals_per_env <= 0:
            raise ValueError("EvalConfig.num_evals_per_env must be positive")
        per_env_limit = max(1, config.num_evals_per_env)
        methods_per_batch = max(1, config.num_evals // per_env_limit)
        method_queue = []

        def record_outputs(method_name, out, artifacts):
        ...
        def flush_queue(force=False):
            ...
            while method_queue and (force or len(method_queue) >= target):
                batch = method_queue[:methods_per_batch]
                del method_queue[:methods_per_batch]
                run_method_batch(batch)

        def eval_and_record(c,method_name,weak_state_dicts=None):
            method_queue.append((c, method_name, weak_state_dicts))
            flush_queue()
