...
class CustomNoisyActionWrapper(wrappers.UnderspecifiedEnvWrapper):
    ...
    def step_env(self, key, state, action, params):
        ...
        action = action + jax.random.normal(key1, action.shape) * self.ACTION_NOISE_STD
...
def eval(
    ...
    env = train_expert.BatchEnvWrapper(
        ...
        wrappers.LogWrapper(wrappers.AutoReplayWrapper(CustomNoisyActionWrapper(env,config.ACTION_NOISE_STD))), config.num_evals
    )
    ...
    def execute_chunk(carry, _):
        def step(carry, action):
            ...
            if PROB:
                ...
                print("env_state type:", type(env_state))
                PROB=False
        ...
        elif isinstance(config.method, GFDMethodConfig):
            prefix_attention_horizon = policy.action_chunk_size - config.execute_horizon
            assert (
                config.inference_delay <= policy.action_chunk_size
                and prefix_attention_horizon <= policy.action_chunk_size
...
def main(
    ...
    # for inference_delay in [0]:
        for execute_horizon in range(max(1, inference_delay), 8 - inference_delay + 1):
            ...
            for a_noise_std in [0.1,0.15,0.2,0.25,0.3,0.35,0.4]:
                #change action noise std
                config = replace(config, ACTION_NOISE_STD=a_noise_std)
                # levels = change_polygon_position_and_velocity(levels, pos_x=1,vel_x=a_noise_std, index=4) #change to vel_y=something here if needed
                ...
                for i in range(len(level_paths)):
                    ...
                    results["action_noise_std"].append(a_noise_std)
            ...
            #         results[k].append(v[i])
            #     results["delay"].append(inference_delay)
            #     results["method"].append("Guided_flow")
            #     results["level"].append(level_paths[i])
            #     results["execute_horizon"].append(execute_horizon)
